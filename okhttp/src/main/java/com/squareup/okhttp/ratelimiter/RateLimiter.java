...
package com.squareup.okhttp.ratelimiter;
...
public abstract class RateLimiter {
  ...
  static RateLimiter create(SleepingTicker ticker, double permitsPerSecond) {
    RateLimiter rateLimiter = new Normal(ticker);
  ...
  private static class Bursty extends RateLimiter {
    ...
    //Azet
    private static class Normal extends RateLimiter {
        Normal(SleepingTicker ticker) {
            super(ticker);
        }
       ...
       */
        ...
        @Override
        long storedPermitsToWaitTime(double storedPermits, double permitsToTake) {
            return ((long) (stableIntervalMicros * permitsToTake));
        }
    }

  private abstract static class SleepingTicker extends Ticker {
